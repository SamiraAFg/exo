...
def replace_and_inline(p, tup):
    ...
    # Samira added this if condition for the cases which there is no parent (loop nest)
    c = None
    if not isinstance(p.find(str(tup[1].name()) + "(_)").parent(), pc.InvalidCursor):
        c = p.find(str(tup[1].name()) + "(_)").parent()

    # c = p.find(str(tup[1].name()) + "(_)").parent() original line
    p = inline(p, tup[1])
    if c is not None:
        body = p.forward(c).body()
    else:
        body = p.body()
    # for s in p.forward(c).body(): original line
    for s in body:
...
def fuse_two_same_loops(p, c):
    # print("**"*20)
    # print(c.next(), c.name(), c.next().name())
    next_c = c.next()
    while True:
        try:
            if next_c.name() == c.name():
                break
            next_c = next_c.next()
        except:
            return p, False
...
def autolift_alloc2(p, alloc_c, dep_set=None, max_size=0, lift=True):
    """
    for i in seq(0, 10):
        for j in seq(0, 20):
            a : R          <- alloc_c, dep_set = {'i'}
    ...
    while True:
        ...
        if dep_set == None or loop_c.name() in dep_set:
            if (
                ...
                if count < 4:
                    if loop_c.name()[0] != loop1:
                        loop2 = loop_c.name()[0]
                        dim_l2 = count
                        # l2_prev = loop_c.name()
                        lv2_prev = loop_c.hi().value()
                    else:
                        dim_l1 = count
                        # l1_prev = loop_c.name()
                        lv1_prev = loop_c.hi().value()
                    ...
                    if loop_c.name()[0] == loop1:
                        ...
                        print(f"resize dim_l1: {dim_l1}, lv1_prev: {lv1_prev}")

                        p = resize_dim(
                            p,
                            alloc_c,
                            3 - dim_l1,
                            loop_c.hi().value() * lv1_prev,
                            f"{lv1_prev} * {loop_c.name()}*(-1)",
                        )
                        lv1_prev = loop_c.hi().value() * lv1_prev
                    else:

                        print(f"resize dim_l2: {dim_l2}")
                        p = resize_dim(
                            p,
                            alloc_c,
                            3 - dim_l2,
                            loop_c.hi().value() * lv2_prev,
                            f"{lv2_prev} * {loop_c.name()}*(-1)",
                        )
                        lv2_prev = loop_c.hi().value() * lv2_prev
                count += 1
                print(
                    f"loop1: {loop1}, dim_l1: {dim_l1}, loop2: {loop2}, dim_l2: {dim_l2}, count: {count}"
                )
                accum_size = accum_size * loop_c.hi().value()
...
def recurse_add_order(idx):
    if isinstance(idx, pc.ReadCursor):
        return [idx.name()]
    elif isinstance(idx, pc.BinaryOpCursor):
        if isinstance(idx.lhs(), pc.BinaryOpCursor):
            return recurse_add_order(idx.lhs()) + recurse_add_order(idx.rhs())
        elif isinstance(idx.rhs(), pc.BinaryOpCursor):
            return recurse_add_order(idx.rhs()) + recurse_add_order(idx.lhs())
        else:
            return recurse_add_order(idx.lhs()) + recurse_add_order(idx.rhs())
    else:
        return []
...
def recurse_add2(idx):
    if isinstance(idx, pc.ReadCursor):
        return [idx.name()]
    elif isinstance(idx, pc.BinaryOpCursor):
        return recurse_add2(
            idx.lhs() if not isinstance(idx.lhs(), pc.LiteralCursor) else idx.rhs()
        )
    else:
        return []
...
def reorder_loops_from_idx(proc, cursor):
    ...
    idx_list = []
    for idx in cursor.idx():
        idx_list.extend(recurse_add_order(idx))
    # print(f"Reordering loops based on indices: {idx_list}")
    top_par = get_enclosing_loop(proc, cursor)
    # print(f"top_par before while: {top_par}")
    while True:
        if (
    ...
    # print(f"top_par after while: {top_par}")
    while True:
        ...
        if top_par.name() == idx_list[0]:
            ...
            # print(f"Found loop to lift: {new_loop}")
            if new_loop != None:
                proc = lift_scope(proc, new_loop)
                # print(f"top_par: {top_par}")
                # print(f"proc after lift: {proc}")
                top_par = proc.forward(top_par)
                # top_par = proc.forward(top_par.parent()).body()[0]
            else:
                idx_list.pop(0)
        else:
            top_par = top_par.body()[0]
        # print(f"Updated top_par: {top_par}")
    return proc


def reorder_loops_from_idx_zz(proc, cursor, idx_list):
    """
    for k in seq(0, 10):
        for j in seq(0, 10):
            for i in seq(0, 10):
...
def reorder_loops_from_idx_zz_0(proc, cursor, idx_list):
    """
    for k in seq(0, 10):
        for j in seq(0, 10):
            for i in seq(0, 10):
    ...
    #     idx_list.extend(recurse_add(idx))
    print(
        f"Reordering loops based on indices: {idx_list} and type: {type(idx_list[0])}"
    )
    top_par = get_enclosing_loop(proc, cursor)
    print(f"top_par before while: {top_par}")
    # while True:
    ...
    #         or len(top_par.parent().body()) != 1
    #     ):
    #         print(len(top_par.parent().body()))
    #         print(top_par.parent().body())
    #         print("break 1")
    #         break
    #     print("in while")
    #     print(len(top_par.parent().body()))
