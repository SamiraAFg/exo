...
class GemminiScheduler(Scheduler):
    def get_load_a_pat(self):
        return ("i", "k"), (1, 1, 16, 16)

    def get_load_b_pat(self):
        return ("k", "j"), (1, 1, 16, 16)
...
def generate_sample3_schedule():
    ...
    # order = ('joo', 'joi', 'ko', 'io', 'ii', 'ji', 'ki') # first generated but the inner loop for j is too small for the instruction. tl mapping witout N set to 16 (we are manipulating this)! in timeloop mapping M -> i, N -> j, K -> k
    order = (
        "jooo",
        "jooi",
        "joi",
    ...
    # order = ('joo', 'ioo', 'koo', 'joi', 'koi', 'ioi', 'ii', 'ji', 'ki') # tl mapping witout N set to 16 (we are manipulating this)! in timeloop mapping M -> i, N -> j, K -> k
    loads = (1, 0, -1)  #'jooo', 'jooi' guards for A
    ii = (16, 16)  # TODO check if the order matches the tilings
    jj = (8, 2, 4, 16)
    kk = (32, 16)
    # order = ('jooo', 'koo', 'jooi', 'koi', 'joi', 'io', 'ii', 'ji', 'ki') # Delay and N and K for accumulator or buffer allocation dim should have N and K more than 4 (the workaround)
    # loads = (0, 1, -1)  # tl mapping (2, 2, 4)
    # ii = (16, 16)
    # jj = (2, 8, 4, 16)
    # kk = (4, 8, 16)
    # mapping with edp and N & k in accumulator should be above 4
    # order = ('joo', 'ioo', 'joi', 'ko', 'ioi', 'ii', 'ji', 'ki') # tl mapping witout N set to 16 (we are manipulating this)! in timeloop mapping M -> i, N -> j, K -> k
    # loads = (0, 0, 1)
    # ii = (4, 4, 16)
    # jj = (4, 16, 16)
    # kk = (32, 16)
    ...
    # ac = gemmini.find("for _ in _:_").find("_(_) #1")
    # gemmini = add_unsafe_guard(gemmini, ac, "jooo == 0")
    # gemmini = add_unsafe_guard(gemmini, gemmini.find("do_ld_i8_block_id1(_)"), "jooo == 0")
    # gemmini = add_unsafe_guard(gemmini, gemmini.find("do_ld_i8_block_id1(_)"), "jooi == 0")
...
def generate_sample2_schedule():
    ...
    loads = (2, 1, -1)  # tl mapping (1, 1, 2) guard ioo for B and jooi guard for A
    ii = (2, 16, 16)
    jj = (4, 2, 4, 16)
    kk = (32, 16)
    workload_size = (N, M, K)
...
def generate_sample1_schedule():
    ...
    # order = ('io', 'ko', 'jo', 'ii', 'ji', 'ki') # zz mapping with N set to 16!
    order = (
        "koo",
        "ioo",
        "joo",
    ...
    loads = (2, -1, -1)  # tl mapping (2, 2, 4) guard ioo for B
    ii = (2, 32, 16)
    jj = (8, 2, 16)
    kk = (2, 16, 16)
    workload_size = (N, M, K)
